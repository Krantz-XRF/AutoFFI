...
{
    if (auto p = cfg.FileMarshallers.find(name);
        p != cfg.FileMarshallers.cend())
    ...
    *os << '\n';
    ...
    if (!mod.imports.empty())
        *os << '\n';
    for (auto& [name, tag] : mod.tags)
        genTag(name, tag);
    for (auto& entity : mod.entities)
        genEntity(entity);
}
                                  ...
                                  const Enumeration& enm) noexcept
{
    ...
    *os << " }";
    *os << '\n';
    for (auto& [item, val] : enm.values)
        genEnumItem(name, item, val);
    *os << '\n';
}
